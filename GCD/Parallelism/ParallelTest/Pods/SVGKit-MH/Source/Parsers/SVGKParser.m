...
{
    NSLog(@"START: newParserWithDefaultSVGKParserExtensions...");

    NSLog(@"Current parsing parser for current thread: [%@]", getCurrentlyParsingParser());

    SVGKParser *parser = [[SVGKParser alloc] initWithSource:source];

    NSLog(@"newParserWithDefaultSVGKParserExtensions Parser created: [%@]", parser);
...
{
    NSLog(@"START: parseSourceUsingDefaultSVGKParser...");

    NSLog(@"Current parsing parser for current thread: [%@]", getCurrentlyParsingParser());

    SVGKParser* parser = [self newParserWithDefaultSVGKParserExtensions:source];

    NSLog(@"parseSourceUsingDefaultSVGKParser Parser created: [%@]", parser);

    SVGKParseResult* result = [parser parseSynchronously];

    NSLog(@"END: FINISHED PARSING SVG! [%@]", result);
...
{
    NSLog(@"SVGKParser parseSynchronously: self.currentParseRun [%@]", self.currentParseRun);
